...
class Solution
{
    ...
    private int[] inorder(Node root){
        ArrayList<Integer> ans = inorderHelper(root);
        
        int[] inordersorted= new int[ans.size()];
        for(int i=0;i<ans.size();i++){
            inordersorted[i]= ans.get(i);
        }
        Arrays.sort(inordersorted);
        return inordersorted;
    }
    
    
    private ArrayList<Integer> inorderHelper(Node root){
        ArrayList<Integer> res= new ArrayList<>();
        if(root==null){
            return res;
        }
        
        res.addAll(inorderHelper(root.left));
        res.add(root.data);
        res.addAll(inorderHelper(root.right));
        return res;
    }
